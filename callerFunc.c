...
/************************ PUT_YOUR_MACRO_HERE *****************************************************************
 * Code copied to git ignored 'myNote' folder
*/
#define PUT_YOUR_MACRO_HERE     "Keyword to search"
#define LOG_PUT_YOUR_MACRO_HERE "The code of the log to put into the file.c"
...
int read_line(char* file_to_open)
{
    ...
    while(NULL != fgets(line_read, MAX_READ_SIZE_PER_LINE, fp))
    {
        ...
        // skip logs line
        // search "skip logs line" in the folder under 'myNote' folder
        if 
        (
            strstr(line_read, "KEYWORD")
        )
        {
            prev_seek = ftell(fp);
            continue;
...
#ifdef PUT_YOUR_MACRO_HERE
        if 
        (
            strstr(line_read, PUT_YOUR_MACRO_HERE) &&
            (NULL == strstr(line_read, PUT_YOUR_MACRO_HERE "_")) && // skip if it whole word is not PUT_YOUR_MACRO_HERE entirely
            (NULL == strstr(line_read, PUT_YOUR_MACRO_HERE ".h")) && // skip if is header file
            (!match(line_read)) &&
            (1 == regexec( &reegex_no_match17, line_read, 0, NULL, 0))
        )
        {
            if(TRUE == is_within_comment(line_read, PUT_YOUR_MACRO_HERE))
                continue;
            ret = write_file_short(fp, file_to_open, temp, buffer, str_to_write, line_read, &prev_seek, &old_file_size, &new_file_size, &bytes_written, LOG_PUT_YOUR_MACRO_HERE);
            if(SUCCESS != ret)
            {
                printf("%s:%d Terminate\n", __FUNCTION__, __LINE__);
                exit(0);
